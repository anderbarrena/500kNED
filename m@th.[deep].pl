...
binmode STDOUT, ":utf8";
my ($COLLECT,$TEST,$W2V,$DICT,$CLUSTER,$TRAIN_M0THS,$PRIOR,$PATH,$SUBSET,$AUGMENTED,$WINDOW,
$TAG,$NN,$CASE,$GENERATIVE,$OUT,$ROUND,$TO_PEC,$PES,$DOC);
...
"SUBSET=s" => \$SUBSET, "AUGMENTED" => \$AUGMENTED, "WINDOW=f" => \$WINDOW, "TAG=s" => \$TAG,
"NN=s" => \$NN, "CASE" => \$CASE, "OUT=s" => \$OUT, "GENERATIVE" => \$GENERATIVE, 
"ROUND=f" => \$ROUND, "TO_PEC" => \$TO_PEC, "PES" => \$PES, "DOC" => \$DOC);
...
sub init
{
    ...
    mkdir $path."ssv/train/mentions/" if $CLUSTER;
    ...
    \t
    \t
    \nDeep Neural Net based Word Expert Entity Linking model at --> ".localtime()."\n";
...
sub mPaths
{
	...
	$test =~ s/\.bz2//g;
	$test .= ".doc" if $DOC;
	$ssv = $path."ssv/test/".$f."/".$s."/".$t."/".$w.".".$test.".".$nn;
	$ssv =~ s/\.py$//g;
	$m0th = $path."m\@ths/".$f."/".$s."/".$t."/".$w.".".$nn;
	$m0th =~ s/\.py$//g;
	&createPaths($path."ssv/test/",$f,$s,$t);
    }
    if($CLUSTER || $TRAIN_M0THS)
    {
	$ssv = $path."ssv/train/mentions/".$f."/".$s."/".$t."/".$w;
	$ssv =~ s/\.py$//g;
	$m0th = $path."m\@ths/".$f."/".$s."/".$t."/".$w.".".$nn;
	$m0th =~ s/\.py$//g;
	&createPaths($path."ssv/train/mentions/",$f,$s,$t);
	&createPaths($path."m\@ths/",$f,$s,$t);
    }
    return ($ssv,$m0th);
}
...
sub cluster
{
	...
	foreach my $ent (keys %{$prior{$w}})
	{
		...
		chomp;
		my $line = $_;
		my $mention = quotemeta($w);
		if($line =~ m/#m#$mention#m#/i)
		{
		    push(@same,$line);
		}
		elsif ($AUGMENTED)
	    ...
	    if ($AUGMENTED)
	    {
		foreach my $instance (shuffle(@rest))
		{	    
		    my $context = &order($instance);
		    next if @$context eq 0;
		    $top++;
...
sub shuffleDoc
{
    ...
    my @words = split(" ",$text);
    my $k = 0;
    my @order = (49,50,48,51,47,52,46,53,45,54,44,55,43,56,42,57,41,58,40,59,39,60,38,61,37,62,36,63,35,64,34,65,33,66,32,67,31,68,30,69,29,70,28,71,27,72,26,73,25,74,24,75,23,76,22,77,21,78,20,79,19,80,18,81,17,82,16,83,15,84,14,85,13,86,12,87,11,88,10,89,9,90,8,91,7,92,6,93,5,94,4,95,3,96,2,97,1,98,0,99);
    foreach my $w (shuffle(@words))
    {
	next if !defined($w2v{$w});
	$res[$order[$k]] = $w2v{$w};
	$k++;
	last if $k eq 100;
    }
...
sub testM0ths
{
	...
	my($docid,$id,$o,$m,$mm,$hm,$type,$w_ctx,$m_ctx,$m_fd) = split (/\t/,$line); 
	my $w = &getW($m,$mm,$hm);
	foreach my $ent (keys %{$prior{$w}}) #init all
	{
	    $r{$id}{$ent} = log($prior{$w}{$ent});
	    $id2w{$id} = $w;
	}
	$r{$id}{"NIL"} = &nilProb();
    }	
	...
	while(<PLG>)
	{
	    ...
	    my ($id,$z) = split(":::",$idun);
	    my $w = $id2w{$id};
	    foreach my $ent (sort {$kode{$w}{$b} <=> $kode{$w}{$a}} keys %{$kode{$w}})
	    {
		my $k = $kode{$w}{$ent};
		my $prob = $res[$k-1];
		$r{$id}{$ent} += $prob/$z if defined($prob);
		# $r{$id}{$ent} += log(1/$M)/$z unless defined($prob);
		$r{$id}{$ent} -= log($prior{$w}{$ent})/$z unless $PRIOR;
	    }
	}
	close(PLG);
    }
