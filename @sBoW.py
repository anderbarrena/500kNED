...
parser.add_option("-v", "--clusters", dest="clusfile",
                  help="clusters ssv file", metavar="<clusters_file>")
...
def to_sparse(x):
    y = torch.zeros(len(x),csize).type(torch.FloatTensor)
    window = len(x[0])
    for l in range(len(x)):
        ...
        if len(z) != 0:
            for w in z:
                pos = x[l][w[0]]
                if pos != 0:
                    y[l][cLsTensor[pos-1][0]] += 1
    return y

if torch.cuda.is_available():
    torch.cuda.set_device(gpuN)

if options.clusfile is not None:
    if options.trainfile is not None:
        print()
        print(" GPU available:",torch.cuda.is_available())
        now = datetime.datetime.now()
        sys.stdout.write(" loading clusters: ")
        sys.stdout.flush()

    cLs = numpy.loadtxt(options.clusfile, skiprows=0, usecols=1, dtype=int, ndmin=2)
    csize = int(numpy.amax(cLs)+1)
    cLsTensor = torch.from_numpy(cLs).type(torch.IntTensor)
    if options.trainfile is not None:
        prev = now
        now = datetime.datetime.now()
...
if options.trainfile is not None:
    ...
    for m in plague:
        ...
        D=None
        G=None
        devDataTensor=to_sparse(torch.from_numpy(devD).type(torch.IntTensor))
        devGoldTensor=torch.from_numpy(devG).type(torch.IntTensor)
        trainDataTensor=to_sparse(torch.from_numpy(trainD).type(torch.IntTensor))
        ...
        for r in [1,2,3]:
            nowT=datetime.datetime.now()
            NNet=DeepNet(csize,hidden,classes)
...
if options.testfile is not None:
    ...
    for m in plague:
        ...
        inputs=to_var(to_sparse(torch.from_numpy(data).type(torch.IntTensor)))
        data=None
