...
class DeepNet(torch.nn.Module):
    def __init__(self,input_size,lstm_hidden_size,feedf_hidden_size,num_classes):
        ...
        self.feedf = torch.nn.Sequential(
            torch.nn.Linear(lstm_hidden_size, feedf_hidden_size),
            torch.nn.ReLU(),
            torch.nn.Dropout(dp),
...
if options.trainfile is not None:
    ...
    for m in plague:
        ...
        for r in [1,2,3]:
            nowT=datetime.datetime.now()
            NNet=DeepNet(vsize,lstm_hidden,feedf_hidden,classes)
